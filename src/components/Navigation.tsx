...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled ? 'bg-slate-800/95 backdrop-blur-md shadow-lg border-b border-slate-700' : 'bg-slate-800/80 backdrop-blur-sm'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-slate-700/95 backdrop-blur-md rounded-lg mx-4 mb-2 border border-slate-600">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
